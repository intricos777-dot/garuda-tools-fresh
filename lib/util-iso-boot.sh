...
prepare_initramfs(){
    local _kernver
    _kernver=$(ls "${1}/usr/lib/modules/" | awk '{print $1}')
    if ${use_dracut}; then
        chroot-run "${1}" \
...
prepare_boot_extras(){
    ...
    cp "${src_root}/boot/amd-ucode.img" "${dst_root}/amd_ucode.img"
    cp "${src_root}/boot/intel-ucode.img" "${dst_root}/intel_ucode.img"
    cp "${src_root}/usr/share/licenses/amd-ucode/LIC"* "${dst_root}/amd_ucode.LICENSE"
    cp "${src_root}/usr/share/licenses/intel-ucode/LIC"* "${dst_root}/intel_ucode.LICENSE"
    cp "${src_root}/boot/memtest86+/memtest.bin" "${dst_root}/memtest"
    cp "${src_root}/usr/share/licenses/spdx/GPL-2.0-only.txt" "${dst_root}/memtest.COPYING"
}
...
prepare_grub(){
    ...
    msg2 "Set menu_show_once=1 in '%s'" "${grub}/grubenv"
